...
SHIPS_HEADER = ['row_id', '公司', 'cell_id', '期间', '出货量', '单位', '推导式', '证据等级', '情景标记',
                '校准实际值', '误差', '校准日期', '检索日期']
SHIPS_ROWS = [
    ['SE001', '甲公司', 'MOD1', '2025年度', '2109', '万只', '年报产销表直接披露(光通信收发模块)', 'B', 'base', '', '', '', '2026-01-01'],
    ['SE002', '乙公司', 'MOD1', '2025年度', '1603', '万只', '年报产销表直接披露(光互联产品)', 'B', 'base', '', '', '', '2026-01-01'],
...
def test_shipment_section_expresses_unit_and_business_scope(env):
    ...
    assert '| SE001 | 甲公司 | MOD1 光模块 | 2025年度 | 2109 万只 | B | base |' in md
    # 聚合行与非 base 情景行有显式标记
    assert '⚠跨层聚合' in md
    assert '⚠非base' in md
